...
By default package uses Standard Pipeline shaders, you can change those to HDRP or URP by taking advantage of Unity material conversion tools.",
        ...
        private static readonly ScrollViewGuiSection MainContentSection = new ScrollViewGuiSection(
            "", (screen) =>
            {
                ...
                GUILayout.Label(
                   @"Ivy Studio is included in the Lunar New Year Mega Bundle until 9th Feb. 23

Ivy Studio can be upgraded to Sky Master ULTIMATE for only $35
and from Sky Master ULTIMATE can upgrade to the Environment Building 
Bundle that contains all ARTnGAME assets for $99 for a limited time

Sky Master ULTIMATE can also be upgraded to all other major ARTnGAME 
assets with big discounts.

In order to enable the offers must first purchase the asset to upgrade 
from and then visit the other asset pages to see the discounted price and 
